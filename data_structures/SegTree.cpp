...
struct SegTree{
    ...
    */
    ...
    void build(vector<T> &a, int v = 1, int lt = 0, int rt = -1){
        if(rt < 0) rt += n;
        if(lt == rt){
            t[v] = Data(a[lt]);
            return;
    ...
    Data query(int l, int r, int v = 1, int lt = 0, int rt = -1){
        if(rt < 0) rt += n;
        if(lt == l && rt == r){
            return t[v];
        }
        ...
        Data ln, rn;
        if(l <= md){
            ln = query(l, min(md,r), 2*v, lt, md);
            lp = &ln;
        }
        if(r >= md+1){
            rn = query(max(md+1,l), r, 2*v+1, md+1, rt);
            rp = &rn;
        }
    ...
    void updateValue(int pos, T &value, int v = 1, int lt = 0, int rt = -1){
        if(rt < 0) rt += n;
        if(lt == rt){
            t[v] = Data(value);
            return;
        ...
        int md = lt + (rt - lt)/2;
        if(pos <= md){
            updateValue(pos, value, 2*v, lt, md);
        }else{
            updateValue(pos, value, 2*v+1, md+1, rt);
        }
        t[v] = Data::combine(&t[2*v], &t[2*v+1]);
    }
};
...
struct SegTreeLazy{
    ...
    */
    ...
    struct Data{
        ...
        Data(T val = T(), T lz = zero()) : val(val) , lz(lz) {}
    ...
    void updateRange(int l, int r, T add, int v = 1, int lt = 0, int rt = -1){
        if(rt < 0) rt += n;
        if(l > r) return;
        push(v, lt, rt);
        if(lt == l && rt == r){
        ...
        int md = lt + (rt - lt)/2;
        updateRange(l, min(md,r), add, 2*v, lt, md);
        updateRange(max(md+1,l), r, add, 2*v + 1, md+1, rt);
        t[v] = Data::combine(&t[2*v], &t[2*v+1]);
    }
};
